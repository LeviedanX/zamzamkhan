    ...
    public function up(): void
    {
        ...
        if (! DB::table('advantages')->exists()) {
            ...
            foreach ($items as $i => [$icon,$title,$description]) {
                DB::table('advantages')->insert(compact('icon', 'title', 'description') + ['display_order' => $i + 1, 'is_active' => true, 'created_at' => $now, 'updated_at' => $now]);
            }
        }
        ...
        if (! DB::table('clients')->exists()) {
            ...
            foreach ($items as $i => [$name,$file]) {
                DB::table('clients')->insert(['name' => $name, 'logo_path' => 'images/Logo/'.$file, 'display_order' => $i + 1, 'is_active' => true, 'created_at' => $now, 'updated_at' => $now]);
            }
        }
        if (! DB::table('testimonials')->exists()) {
            for ($i = 1; $i <= 11; $i++) {
                DB::table('testimonials')->insert(['client_name' => 'Dokumentasi Klien '.$i, 'service_name' => 'Sertifikasi Halal', 'content' => 'Dokumentasi pendampingan dan penyerahan sertifikat halal bersama PT Zam Zam Khan.', 'image_path' => 'images/testimonials/testi'.$i.'.jpeg', 'image_alt' => 'Dokumentasi pendampingan klien '.$i, 'display_order' => $i, 'is_active' => true, 'created_at' => $now, 'updated_at' => $now]);
            }
        }
